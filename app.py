...
def main():
    ...
    if "flight_hb_seq_list" not in st.session_state:
        st.session_state.flight_hb_seq_list = []   # 心跳序号列表
        st.session_state.flight_hb_time_list = []  # 心跳时间列表
    ...
    # ==================== 飞行监控页面（手动步进版本） ====================
        ...
        # 手动控制刷新：移除 st_autorefresh，改为用户点击“步进”按钮来推进飞行和心跳
        ...
        with col_ctrl:
            ...
            with c4:
                if st.button("重置", use_container_width=True):
                    ...
                    st.rerun()
            with c5:
                # 步进按钮：手动推进飞行一步，并生成一个心跳包
                step_clicked = st.button("⏩ 步进", use_container_width=True)
                if step_clicked and st.session_state.simulation_running and not st.session_state.heartbeat_sim.paused:
                    # 执行一次飞行更新
        ...
        if st.session_state.simulation_running and not st.session_state.heartbeat_sim.paused and time_since_last > 3.0:
            st.error("🚨 连接超时！超过3秒未收到心跳包（请点击步进）")
